...
const solution = input => {
  ...
  const addToCompleteTrails = (startCoord, endCoord) => {

    if (!completeTrails[startCoord]) {
      completeTrails[startCoord] = new Set();
    }
    completeTrails[startCoord].add(endCoord);
  }
    ...
    while (true) {
      ...
      if (nextCoords.length === 1) {
        ...
        if (grid[nextCoords[0].x][nextCoords[0].y] === 9) {
          ...
          addToCompleteTrails(trail.startCoord, getSerialisedCoords(nextCoords[0]));
          oneTrail.splice(oneTrail.indexOf(trail), 1);
          break;
        }
      } else {
        ...
        for (const nextCoord of nextCoords) {
          ...
          if (grid[nextCoord.x][nextCoord.y] === 9) {
            addToCompleteTrails(trail.startCoord, getSerialisedCoords(nextCoord));
          } else {
  ...
  return Object.entries(completeTrails).reduce((acc, [startCoord, endCoordsSet]) => {
    return acc + endCoordsSet.size;
  }, 0);
};
